...
export async function generateMetadata({ params }: PageProps<"/projects/[slug]">): Promise<Metadata> {
  ...
  const project = (await getPublicProjects()).find(item => item.slug === slug);
  return project ? { title: project.title, description: project.summary, alternates: { canonical: `/projects/${project.slug}` }, openGraph: { title: project.title, description: project.summary, url: `/projects/${project.slug}`, images: project.image ? [project.image] : undefined } } : { title: "Projet introuvable", robots: { index: false } };
}
