...
export default async function ProjectDetailPage({ params }: PageProps<"/projects/[slug]">) {
  ...
  return (
    <PageContainer className="route-page project-detail">
      ...
      <section className="project-detail-grid"><div><p className="system-label">Mission</p><h2>Ce que le système cherche à résoudre.</h2></div><div><p>{project.description}</p><blockquote>{project.outcome}</blockquote><ul className="tech-list">{project.tags.map((tag) => <li key={tag}>{tag}</li>)}</ul>{project.demoUrl ? <a className="primary-action" href={project.demoUrl} target="_blank" rel="noreferrer">Ouvrir la démonstration <ArrowUpRight size={17} /></a> : <p className="availability-note">Aucune démonstration publique n’est annoncée pour ce niveau d’avancement.</p>}</div></section>
